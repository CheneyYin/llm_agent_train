...
基础篇和进阶篇合并，8 个技巧连贯讲解。用一个贯穿式的真实任务，逐步叠加技巧，展示输出质量如何一步步提升。
...
定位：Extension 是 pi 的插件框架。补充 Part 3 中 registerTool 的底层机制，讲解 ExtensionAPI 完整能力。

| 时间 | 主题 | 内容 | 演示 |
|------|------|------|------|
| 1分钟 | Extension 是什么 | TypeScript 入口 + 自动扫描路径 | — |
| 2分钟 | ExtensionAPI 核心能力 | 6 个核心 API + 生命周期 Mermaid 图 | 表格展示 |
| 1分钟 | 回到 log-tools.ts | 从 Extension 视角重新审视已有代码 | 编辑器展示 |

< 衔接过渡 4 → 5 >
"Tool 和 Extension 给了 Agent 执行能力。但提示词本身的质量怎么保证？"
...
定位：合并原 Part 2 技巧八的"封装复用"视角和 agentskill.io 规范，完整讲解 Skill 的设计、加载与生命周期。Skill 是 Agent 的知识库，Tool 是执行器。

| 时间 | 主题 | 内容 | 演示 |
|------|------|------|------|
| 2分钟 | Skill vs Tool | 对比表：作用/注入位置/触发方式/生命周期 | — |
| 3分钟 | agentskill.io 规范 & 渐进式加载 | 三层加载（metadata→指令→资源），Mermaid 图 | 展示 skills/log-analyzer/ 目录结构 |
| 2分钟 | 实战演示 | pi 中演示 Skill 完整生命周期：启动扫描→匹配加载→执行 | pi 终端演示 |
| 1分钟 | 封装一个 Skill | 回到技巧八核心洞察：把反复使用的提示词封装为 Skill | — |
...
| 时间 | 内容 |
|------|------|
| 2分钟 | **核心要点回顾**：四句话总结——① 好提示词 = 清晰角色 + 结构化要求 + 示例 ② 进阶技巧让 AI 自己思考验证 ③ Tool = Agent 的手 + Skill = Agent 的知识库 ④ Agent = 自动化的"思考→行动→观察"循环 |
...
灵活用于超时、额外问答或延长的演示。（总时长 66 分钟，缓冲 2 分钟）
