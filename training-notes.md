...
**核心类比**：给一个初级程序员布置任务。你说得越清楚、越具体，他做得越好。

### 什么是提示词工程（2 分钟）

提示词工程听起来高大上，其实就是**学会怎么跟 AI 说话**。

想象你刚从学校招了一个实习生，他很聪明但完全不了解你的项目。你跟他说"帮我改一下代码"，他肯定一脸茫然。但如果你说"你是我们团队的 Python 后端开发，负责维护日志解析模块。现在 errors.log 里有几个报错，帮我按错误类型分类统计"，他马上就能上手。

AI 也一样。你不给它上下文，它就瞎猜。你给它清晰的角色和任务描述，它就表现得好。

**过渡**：所以提示词工程听起来高大上，其实就是"学会怎么跟 AI 说话"。那我们从头开始——如果你只能做一件事来改进提示词，那就是给 AI 一个身份。

### 技巧一：角色设定（3 分钟）

**类比**：给 AI 一张身份卡。你去看医生和去修车，穿白大褂的人给你的建议你信的程度完全不同——即使他们是同一个人。
...
**类比**：给实习生写需求文档。你不说截止时间他就慢慢做，你不说格式他就按自己喜好来。对 AI 也一样——你不约束它就自由发挥。
...
**类比**：给 AI 看参考答案。老师出题的时候，如果先给一道例题，学生就清楚要答成什么样。AI 也一样——给它 1-2 个输入输出的例子，比写 100 字描述更有效。
...
**类比**：填表单 vs 写作文。你填一个表格，每个格子有明确的含义。你写一段散文，别人可能理解偏了。让 AI 输出 JSON，就像让它填表。
...
**类比**：考试的时候，老师叫你"把解题步骤写出来"。你只写答案不写步骤，错了没法检查，对了也不知道怎么对的。AI 也一样——你让它直接给你答案，它可能猜。你让它一步步思考，每一步都对整体才对。
...
**类比**：排查线上故障的流程——你先看监控（观察），发现异常（思考），然后查日志（行动），看到报错信息（观察），再推测根因（思考），最后重启服务（行动）。这个"思考→行动→观察→再思考"的循环就是 ReAct 模式，它是所有 AI Agent 的底层工作方式。
...
**类比**：和同事聊了三个小时，开头说的重点他早忘了。AI 也一样——每次对话能"记住"的内容是有限的。这个限制叫做**上下文窗口**（Context Window）。理解上下文是怎么组成的、每部分什么时候生效、什么时候消失，是高效使用 Agent 的关键。
...
**类比**：代码里的函数封装。你有一段逻辑用了三次，就会把它抽成函数。好用的提示词也一样——写成模板，下次直接用。
...
- **提示词工程**：你是一个管理者，每次都要给实习生（AI）写详细的任务说明，看他的产出，再决定下一步让他做什么。
- **Agent**：你把整个项目的目标和可用工具交给它，它自己循环"思考→行动→观察→再思考"，直到任务完成。
...
**类比**：自动驾驶也需要司机接管。L4 自动驾驶在高速上很稳，但进入施工路段、天气恶劣、地图失效时，它会提示司机接管。Agent 也一样。
...
1. **好提示词 = 清晰角色 + 结构化要求 + 示例**——把 AI 当成一个聪明但新来的实习生
...
| 概念 | 一句话解释 | 类比 |
|------|-----------|------|
| 提示词工程 | 学会怎么跟 AI 说话 | 给新人布置任务 |
| 角色设定 | 给 AI 一个身份卡 | 医生 vs 修车师傅 |
| 明确指令 | 告诉 AI 要什么、不要什么 | 给实习生写需求文档 |
| Few-shot | 给 AI 看参考答案 | 老师先讲例题 |
| 结构化输出 | 让 AI 输出 JSON 而非散文 | 填表单 vs 写作文 |
| 思维链 CoT | 让 AI 把推理步骤写出来 | 考试写解题过程 |
| ReAct | 思考→行动→观察→再思考 | 排查线上故障 |
| 上下文管理 | 关键信息放前面，定期总结 | 长会议需要 recap |
| 模板化 | 好提示词封装复用 | 代码里的函数 |
| Agent | 自动化的"思考+行动+观察"循环 | 自动驾驶 vs 手动挡 |
